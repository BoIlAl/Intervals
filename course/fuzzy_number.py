...
class FuzzyNumber:
    def __init__(self, peak=None, left=None, right=None, offset: float = 1., levels_count: int = 2):
        ...
        if isinstance(peak, INTERVAL_TYPE):
            self._left[-1] = peak.left
            self._right[-1] = peak.right
    ...
    @property
    def support(self):
        support = self.alpha_level(0)
        return Interval(support[0], support[1])

    @property
    def kernel(self):
        kernel = self.alpha_level(1)
        return Interval(kernel[0], kernel[1])
    ...
    def __add__(self, other):
        if isinstance(other, BASE_TYPE):
            return FuzzyNumber(left=self._left + other, right=self._right + other)
        ...
        return FuzzyNumber(left=self_left + other_left, right=self_right + other_right)
    ...
    def __sub__(self, other):
        ...
        self_left, self_right = self.get_borders(max_levels)
        other_left, other_right = other.get_borders(max_levels)
        return FuzzyNumber(left=self_left - other_right, right=self_right - other_left)
    ...
    def __mul__(self, other):
        ...
        return FuzzyNumber(left=np.minimum.reduce(mult), right=np.maximum.reduce(mult))
    ...
    def __truediv__(self, other):
        if isinstance(other, BASE_TYPE):
            ...
            div = [self_left / other_left,
                   ...
                   self_right / other_left,
                   self_right / other_right]
        return FuzzyNumber(left=np.minimum.reduce(div), right=np.maximum.reduce(div))
    ...
    def __rsub__(self, other):
        if isinstance(other, BASE_TYPE):
            return FuzzyNumber(left=other - self._right, right=other - self._left)
        max_levels = max(self.levels_count, other.levels_count)
        self_left, self_right = self.get_borders(max_levels)
        other_left, other_right = other.get_borders(max_levels)
        return FuzzyNumber(left=other_left - self_right, right=other_right - self_left)
    ...
    def __str__(self):
        return f"[{self.kernel}, {self.support}]"
